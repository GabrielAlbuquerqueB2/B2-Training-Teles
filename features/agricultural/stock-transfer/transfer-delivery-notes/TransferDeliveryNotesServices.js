...
export async function getTransferDeliveryNoteReport(id, user) {

    const apiGatewayData = {
        params: { DocCode: 'DLN20009' },
        data: [{
            "name": "DocKey@",
            "type": "xsd:decimal",
            "value": [[id]]
...
export async function getVarietiesByItemCode(itemCode) {


    const query = new Api()
        .setMethod('GET')
        .setUrl(`/$crossjoin(Items,Cultivation)?$expand=Items($select=ItemCode,U_B2AG_VarietyDescription),Cultivation($select=Code)&$filter=Items/U_B2AG_CultivationCode eq Cultivation/Code and Cultivation/U_B2AG_ItemCode eq '${itemCode}'&$orderby=Items/U_B2AG_VarietyDescription`)
        .get()
...
function mapVarietiesToSelect(varieties) {
    return varieties.map(item => {
        return {
            value: '' + item.Items.U_B2AG_VarietyDescription,
            description: item.Items.U_B2AG_VarietyDescription,
            cultivation: item.Cultivation.Code
        }
    })
}
