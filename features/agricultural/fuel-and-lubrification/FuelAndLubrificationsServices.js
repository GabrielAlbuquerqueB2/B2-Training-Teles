    ...
    export async function getFuelInventoryGenExits(branchesIds, filters) {
        ...
        const query = new Api()
            ...
            .setData({
                ...
                QueryOption: `$expand=InventoryGenExits($select=DocEntry,DocNum,DocDate,Comments,U_B2AG_Crop,U_B2AG_Equipment,U_B2AG_Odometer,U_B2AG_Operator,BPL_IDAssignedToInvoice),InventoryGenExits/DocumentLines($select=ItemCode,ItemDescription,Quantity),Warehouses($select=WarehouseName,BusinessPlaceID)&$filter=${filterQuery}&$orderby=InventoryGenExits/DocDate desc`
            })
            .get()
